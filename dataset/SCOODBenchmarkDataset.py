...
class SCOODDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, id_name, ood_name, transform):
        ...
        imglist_path = os.path.join(root, 'imglist/benchmark_%s' % id_name, 'test_%s.txt' % ood_name)
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.root, 'images', image_name)
        with open(img_path, 'rb') as f:
            picture = Image.open(f).convert('RGB')
        if self.transform is not None:
            img = self.transform(picture)
